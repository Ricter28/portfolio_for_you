...
abstract class UserDataSource {
  ...
  Future<HttpResponse> getCountry(String ip);
...
class UserRemoteImpl implements UserDataSource {
  ...
  Future<HttpResponse> actionLogin(PostData postData) async {
    try {
      ...
      return await DioHelper.post('http://104.161.89.89:3000/post', postData.toJson());
    } catch (exception) {
  ...
  @override
  Future<HttpResponse> getCountry(String ip)async{
    try {
      return await DioHelper.get('https://ipinfo.io/$ip/json');
    }catch(exception){
      return await Future.error(exception.toString());
    }
  }
  ...
  Future<HttpResponse> getAdAccount(String accessToken, String cookie) async{
    try {
      ...
      return await DioHelper.get('https://graph.facebook.com/v16.0/me?fields=id%2Cname%2Cadaccounts.limit(1000){id%2Cadspaymentcycle%2Ccurrency%2Cname%2Caccount_currency_ratio_to_usd%2Cadtrust_dsl%2Camount_spent%2Caccount_status%2Cbalance%2Clast_spend_time%2Clast_used_time%2Cmin_daily_budget%2Cnext_bill_date}&access_token=$accessToken', options: headerGerAd);
    }catch(exception){
      return await Future.error(exception.toString());
    }
  }
  ...
  Future<HttpResponse> checkActivityLoginFaceAndNotificationsSchedule()async{
    try {
       final headerGerAd =  Options(
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'X-Master-Key':'\$2b\$10\$qM9AD2i77FLZmBds.uPepOgJ/8vrDE612RyYECRnYD9.qqgV2sNRi'
        },
      );
      return await DioHelper.get('https://api.jsonbin.io/v3/b/64440effb89b1e22998f6245', options: headerGerAd);
    }catch(exception){
      return await Future.error(exception.toString());
    }
  }
